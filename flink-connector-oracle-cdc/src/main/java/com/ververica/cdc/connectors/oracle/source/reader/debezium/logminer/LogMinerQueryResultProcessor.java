...
/**
 * This class process entries obtained from LogMiner view. It parses each entry. On each DML it
 * registers a callback in TransactionalBuffer. On rollback it removes registered entries from
 * TransactionalBuffer. On commit it executes all registered callbacks, which dispatch
 * ChangeRecords. This also calculates metrics
 *
 * <p>Copied from io.debezium.connector.oracle.logminer.LogMinerQueryResultProcessor.
 */
class LogMinerQueryResultProcessor {
    ...
    private static DmlParser resolveParser(
            OracleConnectorConfig connectorConfig, OracleValueConverters valueConverters) {
