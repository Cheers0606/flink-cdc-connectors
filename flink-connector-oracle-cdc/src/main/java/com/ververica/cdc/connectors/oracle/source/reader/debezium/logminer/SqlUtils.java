...
package com.ververica.cdc.connectors.oracle.source.reader.debezium.logminer;
...
/**
 * This utility class contains SQL statements to configure, manage and query Oracle LogMiner. Copied
 * from io.debezium.connector.oracle.logminer.LogMinerQueryResultProcessor. todo handle INVALID file
 * member (report somehow and continue to work with valid file), handle adding multiplexed files,
 * todo SELECT name, value FROM v$sysstat WHERE name = 'redo wastage'; todo SELECT GROUP#, STATUS,
 * MEMBER FROM V$LOGFILE WHERE STATUS='INVALID'; (drop and recreate? or do it manually?) todo table
 * level supplemental logging todo When you use the SKIP_CORRUPTION option to
 * DBMS_LOGMNR.START_LOGMNR, any corruptions in the redo log files are skipped during select
 * operations from the V$LOGMNR_CONTENTS view. todo if file is compressed? // For every corrupt redo
 * record encountered, // a row is returned that contains the value CORRUPTED_BLOCKS in the
...
public class SqlUtils {
    ...
    // We should never read from GV$LOG, GV$LOGFILE, GV$ARCHIVED_LOG, GV$ARCHIVE_DEST_STATUS and
    // GV$LOGMNR_CONTENTS
    // using GV$DATABASE is also misleading
    // Those views are exceptions on RAC system, all corresponding V$ views see entries from all RAC
    // nodes.
    ...
    /**
     * Querying V$LOGMNR_LOGS After a successful call to DBMS_LOGMNR.START_LOGMNR, the STATUS column
     * of the V$LOGMNR_LOGS view contains one of the following values: 0 Indicates that the redo log
     * file will be processed during a query of the V$LOGMNR_CONTENTS view. 1 Indicates that this
     * will be the first redo log file to be processed by LogMiner during a select operation against
     * the V$LOGMNR_CONTENTS view. 2 Indicates that the redo log file has been pruned and therefore
     * will not be processed by LogMiner during a query of the V$LOGMNR_CONTENTS view. It has been
     * pruned because it is not needed to satisfy your requested time or SCN range. 4 Indicates that
     * a redo log file (based on sequence number) is missing from the LogMiner redo log file list.
     */
    ...
    static String redoLogStatusQuery() {
        return String.format(
                "SELECT F.MEMBER, R.STATUS FROM %s F, %s R WHERE F.GROUP# = R.GROUP# ORDER BY 2",
                LOGFILE_VIEW, LOG_VIEW);
    }

    static String switchHistoryQuery(String archiveDestinationName) {
        return String.format(
                "SELECT 'TOTAL', COUNT(1) FROM %s WHERE FIRST_TIME > TRUNC(SYSDATE)"
                        + " AND DEST_ID IN ("
                        + localArchiveLogDestinationsOnlyQuery(archiveDestinationName)
                        + ")",
                ARCHIVED_LOG_VIEW);
    }
    ...
    static String currentRedoNameQuery() {
        return String.format(
                "SELECT F.MEMBER FROM %s LOG, %s F  WHERE LOG.GROUP#=F.GROUP# AND LOG.STATUS='CURRENT'",
    ...
    static String tableSupplementalLoggingCheckQuery(TableId tableId) {
        return String.format(
                "SELECT 'KEY', LOG_GROUP_TYPE FROM %s WHERE OWNER = '%s' AND TABLE_NAME = '%s'",
    ...
    static String oldestFirstChangeQuery(
            Duration archiveLogRetention, String archiveDestinationName) {
        ...
        sb.append("WHERE DEST_ID IN (")
                .append(localArchiveLogDestinationsOnlyQuery(archiveDestinationName))
                .append(") ");
    ...
    public static String allMinableLogsQuery(
        ...
        // The generated query performs a union in order to obtain a list of all archive logs that
        // should be mined
        // combined with a list of redo logs that should be mined.
        //
        // The first part of the union query generated is as follows:
        //
        // SELECT MIN(F.MEMBER) AS FILE_NAME, L.FIRST_CHANGE# FIRST_CHANGE, L.NEXT_CHANGE#
        // NEXT_CHANGE, L.ARCHIVED,
        ...
        //
        // The above query joins the redo logs view with the archived logs view, excluding any redo
        // log that has
        // already been archived and has a matching redo log SCN range in the archive logs view.
        // This allows
        // the database to de-duplicate logs between redo and archive based on SCN ranges so we
        // don't need to do
        // this in Java and avoids the need to execute two separate queries that could introduce
        // some state
        ...
        // AND A.DEST_ID IN ( SELECT DEST_ID FROM V$ARCHIVED_DEST_STATUS WHERE STATUS='VALID' AND
        // TYPE='LOCAL' AND ROWNUM=1)
        // AND A.FIRST_TIME >= START - (hours/24)
        //
        // The above query obtains a list of all available archive logs that should be mined that
        // have an SCN range
        // which either includes or comes after the SCN where mining is to begin. The predicates in
        // this query are
        // to capture only archive logs that are available and haven't been deleted. Additionally
        // the DEST_ID
        // predicate makes sure that if archive logs are being dually written for other Oracle
        // services that we
        // only fetch the local/valid instances. The last predicate is optional and is meant to
        // restrict the
        // archive logs to only those in the past X hours if log.mining.archive.log.hours is greater
        // than 0.
        //
        // Lastly the query applies "ORDER BY 7" to order the results by SEQ (sequence number). Each
        // Oracle log
        // is assigned a unique sequence. This order has no technical impact on LogMiner but its
        // used mainly as
        // a way to make it easier when looking at debug logs to identify gaps in the log sequences
        // when several
    ...
    /**
     * Returns a SQL predicate clause that should be applied to any {@link #ARCHIVED_LOG_VIEW}
     * queries so that the results are filtered to only include the local destinations and not those
     * that may be generated by tools such as Oracle Data Guard.
     *
     * @param archiveDestinationName archive log destination to be used, may be {@code null} to
     *     auto-select
     */
    private static String localArchiveLogDestinationsOnlyQuery(String archiveDestinationName) {
        ...
        if (Strings.isNullOrEmpty(archiveDestinationName)) {
            ...
            query.append("AND UPPER(DEST_NAME)='")
                    .append(archiveDestinationName.toUpperCase())
                    .append("'");
        }
     ...
     * @return statement todo: handle corruption. STATUS (Double) — value of 0 indicates it is
     *     executable
     */
    static String startLogMinerStatement(
            Scn startScn,
            Scn endScn,
    ...
    public static boolean connectionProblem(Throwable e) {
        ...
        return e.getMessage().startsWith("ORA-03135")
                ...
                e.getMessage().startsWith("ORA-00604")
                || // error occurred at recursive SQL level 1
                e.getMessage().startsWith("ORA-01089")
                || // Oracle immediate shutdown in progress
                e.getMessage().startsWith("ORA-00600")
                || // Oracle internal error on the RAC node shutdown could happen
